...
def rff_weights(d_old, d_new, seed=0):
    w = tf.random_normal([d_old, d_new], mean=0.0, stddev=1.0, dtype=tf.float32, seed=seed)
    return w


def rff_biases(d_new, seed=100000):
    b = tf.random_uniform([d_new], minval=0.0, maxval=2 * np.pi, dtype=tf.float32, seed=seed)
    return b


def rff_features(x, w, b, theta):
    s = theta[0]
    l = theta[1:]
    d_new = tf.cast(tf.shape(w)[1], tf.float32)
    z = s * tf.sqrt(2. / d_new) * tf.cos(tf.matmul(tf.divide(x, l), w) + b)
    return z
...
class MNISTLinearKernelEmbeddingClassifier():
    ...
    def features(self, x):
        ...
        with tf.name_scope('features'):
            ...
            with tf.name_scope('random_fourier_features'):

                d_old = 1024
                d_new = 1024
                w = rff_weights(d_old, d_new, seed=0)
                b = rff_biases(d_new, seed=1000000)
                theta = tf.ones(2, dtype=tf.float32)
                z = rff_features(h_fc_1_dropout, w, b, theta)
